    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === 'all') {
            ...
            if (roleToAdd.permissions.has("KickMembers") || roleToAdd.permissions.has("BanMembers") || roleToAdd.permissions.has("Administrator") || roleToAdd.permissions.has("ManageChannels") || roleToAdd.permissions.has("ManageGuild") || roleToAdd.permissions.has("MentionEveryone") || roleToAdd.permissions.has("ManageRoles") || roleToAdd.permissions.has("ManageWebhooks") || roleToAdd.permissions.has("ManageEvents") || roleToAdd.permissions.has("ModerateMembers") || roleToAdd.permissions.has("ManageEmojisAndStickers")) {
                return message.channel.send({ 
                    embeds: [client.util.embed()
                        .setDescription(`${client.emoji.cross} | This <@&${roleToAdd.id}> Role is having Dangerous Permissions So i won't give it to everyone \n ROLE PERMISSIONS : ${new PermissionsBitField(roleToAdd.permissions.bitfield).toArray().filter(a => array.includes(a)).map(arr => `\`${arr}\``).join(", ")} permissions`)
                            ...
                            try {
                                ...
                                if (message.guild.roleAssignmentActive && message.guild.roleToAssign) {
                                    for (const m of members) {
                                        const member = m[1];
                                        if (message.guild.roles.cache.has(roleToAdd.id) && 
                                            !member.roles.cache.has(message.guild.roleToAssign.id) && 
                                            !message.guild.roleToAssign.permissions.has("KickMembers") && 
                                            !message.guild.roleToAssign.permissions.has("BanMembers") && 
                                            !message.guild.roleToAssign.permissions.has("Administrator") && 
                                            !message.guild.roleToAssign.permissions.has("ManageChannels") && 
                                            !message.guild.roleToAssign.permissions.has("ManageGuild") && 
                                            !message.guild.roleToAssign.permissions.has("MentionEveryone") && 
                                            !message.guild.roleToAssign.permissions.has("ManageRoles") && 
                                            !message.guild.roleToAssign.permissions.has("ManageWebhooks") && 
                                            !message.guild.roleToAssign.permissions.has("ManageEvents") && 
                                            !message.guild.roleToAssign.permissions.has("ModerateMembers") && 
                                            !message.guild.roleToAssign.permissions.has("ManageEmojisAndStickers")) {
                                            try {
                                    ...
                                    const successEmbed = client.util.embed()
                                        .setColor(client.color)
                                ...
                                return message.channel.send({
                                    embeds: [
                                        embed.setDescription(
                                            `${client.emoji.cross} | An error occurred during role assignment.`
                                        ).setColor(client.color),
                                    ],
                                });
                            }
                        ...
                        interactionMessage.edit({
                            embeds: [
                                client.util.embed()
                                    ...
                                    .addFields({ 
                                        name: 'Reason for Timeout', 
                                        value: 'Your response took longer than expected, exceeding the 10-second time limit.'
                                    })
                                    .setTimestamp()
                            ],
                            components: [],
            ...
            if (membersWithoutRole.size === 0) {
                return message.channel.send({
                    embeds: [
                        client.util.embed().setColor(client.color).setDescription(`All human members already have the provided role: ${roleToAdd.name}`)
                    ]
                });
            }
            ...
            if (roleToAdd && !message.guild.roleAssignmentActive) {
                ...
                interactionMessage.edit({
                    embeds: [
                        embed.setDescription(
                            `**Are you sure you want to give ${roleToAdd} to all humans in this server?**`
                        ),
                    ], 
                    components: [row],
                });
                    ...
                    if (interaction.isButton()) {
                        if (interaction.customId === 'yes_button') {
                            ...
                            interactionMessage.edit({
                                embeds: [
                                    embed.setDescription(
                                        `${client.emoji.tick} | Successfully started adding <@&${roleToAdd.id}> to all human members.`
                                    ),
                                ],
                                components: [],
                            });
                                ...
                                if (message.guild.roleAssignmentActive && message.guild.roleToAssign) {
                                    for (const m of members) {
                                        ...
                                        if (message.guild.roles.cache.has(roleToAdd.id) && 
                                            ...
                                            try {
                                                await client.util.sleep(1400);
                                                await member.roles.add(message.guild.roleToAssign.id, `Role Humans Command Executed By: ${message.author.tag} | ${message.member.id}`);
                                                num++;
                                            } catch (err) {
                                                if (err.code === 429) {
                                                    await client.util.handleRateLimit();
                                    ...
                                    const successEmbed = client.util.embed()
                                        .setColor(client.color)
                                        .setTitle('Role Assignment Successful')
                                        .setDescription(`Successfully assigned the role **${message.guild.roleToAssign.name}** to ${num} human members who previously lacked it`)
                                        .setTimestamp();
            ...
            if (message.guild.roleAssignmentActive && message.guild.roleToAssign) {
                ...
                if (!role) {
                    return message.channel.send({
                        embeds: [
                            embed.setDescription(
                                `${client.emoji.cross} | The role being assigned no longer exists.`
                            ).setColor(client.color),
                        ],
                    });
                }
                ...
                await message.channel.send({ 
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`There is no ongoing role process available`)
                    ] 
                });
            }
        } else {
                    ...
                    return message.channel.send({
                        embeds: [
                            embed
                                .setColor(client.color)
                                .setDescription(
                                    `${client.emoji.tick} | Successfully removed ${role} from ${member}.`
                                )
                        ]
                    });
                } catch (error) {
                    ...
                    return message.channel.send({
                        embeds: [
                            embed
                                .setColor(client.color)
                                .setDescription(
                                    `${client.emoji.cross} | Failed to remove role. An error occurred.`
                                )
                        ]
                    });
                }
                    ...
                    return message.channel.send({
                        embeds: [
                            embed
                                .setColor(client.color)
                                .setDescription(
                                    `${client.emoji.tick} | Successfully added ${role} to ${member}.`
                                )
                        ]
                    });
                } catch (error) {
                    ...
                    return message.channel.send({
                        embeds: [
                            embed
                                .setColor(client.color)
                                .setDescription(
                                    `${client.emoji.cross} | Failed to add role. An error occurred.`
                                )
                        ]
                    });
                }
