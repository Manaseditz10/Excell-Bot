...
const { setPrefix } = require('../../structures/util.js');
            ...
            .addFields([
                {
                    ...
                    value: `To Enable Antinuke, Use - \`${config.prefix}antinuke enable\``
                },
                {
                    name: `__**Antinuke Disable**__`,
                    value: `To Disable Antinuke, Use - \`${config.prefix}antinuke disable\``
                }
            ]);
        ...
        if (!option) {
            ...
            if (isActivatedAlready) {
                ...
                const steps = [
                    ...
                    "Checking Excel's role position for optimal configuration...",
                    'Crafting and configuring the Excel Safety Lock role...',
                    'Ensuring precise placement of the Excel Safety Lock role...',
                ...
                if (message.guild.roles.cache.size > 249)
                    return message.reply(
                        `I Won't Able To Create \`Excel Safety Lock\` Cause There Are Already 249 Roles In This Server`
                    );
                let role = message.guild.members.cache.get(client.user.id)
                    .roles.highest.position;
                let createdRole = await message.guild.roles.create({
                    name: 'Excel Safety Lock',
                    position: role ? role : 0,
                    reason: 'Excel Role For Ubypassable Setup',
